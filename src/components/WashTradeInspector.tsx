...
import { investigate, resolveInput, type Investigation } from "@/lib/investigate";
...
export function WashTradeInspector() {
  ...
  const run = useCallback(async (raw: string) => {
    ...
    setState({ kind: "running", steps: [{ step: "Resolving wallet" }] });
    try {
      ...
      if (runId.current === id) setState({ kind: "done", inv });
    } catch (err) {
      if (runId.current === id) setState({ kind: "error", message: err instanceof Error ? err.message : String(err) });
    }
  }, []);
  ...
  return (
    <>
      ...
      <main className="shell">
        ...
        {state.kind === "running" ? <Running steps={state.steps} /> : null}
        {state.kind === "error" ? (
          <div className="error" role="alert">
            <strong>Could not complete the lookup.</strong> {state.message}
          </div>
        ) : null}
        {state.kind === "done" ? <Report key={state.inv.generatedAt} inv={state.inv} /> : null}
      </main>
    </>
  );
}
...
function Running({ steps }: { steps: Progress[] }) {
  return (
...
function Report({ inv }: { inv: Investigation }) {
